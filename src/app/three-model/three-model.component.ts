...
export class ThreeModelComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private readonly minSceneRadius = 0.65;
  private readonly radiusPadding = 1.2;
  private readonly verticalFillRatio = 0.52;
  private readonly focusDistanceFactor = 0.45;
  private readonly minFocusDistance = 1.75;
  private readonly selectionScaleMultiplier = 1.45;
  ...
  private initScene(): void {
    ...
    const safeRadius = Math.max(this.sceneRadius, this.minSceneRadius);
  ...
  private updateCameraFrustum(width?: number, height?: number): void {
    ...
    const radius = Math.max(this.sceneRadius, this.minSceneRadius);
    const aspect = viewWidth / viewHeight;
    const halfSize = radius / this.verticalFillRatio;
  ...
  private recenterAndFrameModel(force = false): boolean {
    ...
    const normalizedRadius = Math.max(this.boundingSphere.radius, this.minSceneRadius);
    this.sceneRadius = normalizedRadius * this.radiusPadding;
  ...
  private startSelection(mesh: THREE.Mesh, config: SectionEvent): void {
    ...
    const focusDistance = Math.max(this.sceneRadius * this.focusDistanceFactor, this.minFocusDistance);
    const targetWorld = this.tempVector.copy(this.cameraDirection).multiplyScalar(focusDistance);
    ...
    new Tween(scaleData, this.tweenGroup)
      .to({ value: this.selectionScaleMultiplier }, 700)
      .easing(Easing.Cubic.Out)
      .onUpdate(({ value }) => {
        mesh.scale.set(baseScale.x * value, baseScale.y * value, baseScale.z * value);
      })
